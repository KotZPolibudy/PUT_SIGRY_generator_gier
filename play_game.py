...
def play_quest(domain_path, pddl_path, json_path, show_pics=False):
    # Load Domain
    ...
    print(
        color_text(
            f"QUEST START: {quest_data.get('name', problem.name).upper()}",
    ...
    while True:
        ...
        if not loc_id:
            print(
                color_text(
                    "Error: Player location could not be determined from state!",
                    COLOR_RED,
                )
            )
            return False
        ...
        if npcs_here:
            ...
            for npc in npcs_here:
                ...
                print(
                    f"  * {npc_name} ({color_text(status_str, COLOR_RED if hostile or not alive else COLOR_GREEN)}){desc_str}"
                )
        ...
        print(
            f"Inventory: {color_text(', '.join(inventory_names) if inventory_names else 'Empty', COLOR_BLUE)}"
        )
        ...
        if not is_still_solvable(domain, problem, state):
            print(
                color_text(
                    "\n[WARNING] You have reached a dead end! This quest is no longer solvable.",
                    COLOR_RED + COLOR_BOLD,
                )
            )
            print(
                color_text(
                    "You can type 'u' to undo, 'r' to restart, or 'q' to quit.",
                    COLOR_YELLOW,
                )
            )
        ...
        try:
            ...
            if 0 <= action_idx < len(applicable_actions):
                ...
                print(
                    color_text(
                        f"\n> You performed: {translate_action(selected_action, quest_data)}",
                ...
                if act_name == "talk":
                    ...
                    char_name = get_obj_name(char_id, quest_data)
                    print(
                        f"You received the {color_text(item_name, COLOR_GREEN)} from {color_text(char_name, COLOR_YELLOW)}."
                    )
                elif act_name == "steal":
                    char_id = args[1]
                    item_name = get_obj_name(args[2], quest_data)
                    char_name = get_obj_name(char_id, quest_data)
                    print(
                        f"You stole the {color_text(item_name, COLOR_RED)} from {color_text(char_name, COLOR_YELLOW)}!"
                    )
                    ...
                    char_name = get_obj_name(char_id, quest_data)
                    print(
                        f"You attacked {color_text(char_name, COLOR_RED)} with {get_obj_name(args[3], quest_data)}! {color_text(char_name, COLOR_RED)} falls to the ground, dead."
                    )
                elif act_name == "loot":
                    char_id = args[1]
                    item_name = get_obj_name(args[2], quest_data)
                    print(
                        f"You looted {color_text(item_name, COLOR_GREEN)} from the corpse of {color_text(get_obj_name(char_id, quest_data), COLOR_RED)}."
                    )
                elif act_name == "pick-up":
                    print(
                        f"You picked up {color_text(get_obj_name(args[1], quest_data), COLOR_GREEN)}."
                    )
                elif act_name == "unlock":
                    print(
                        f"You unlocked the way to {color_text(get_obj_name(args[2], quest_data), COLOR_CYAN)} using {color_text(get_obj_name(args[3], quest_data), COLOR_GREEN)}."
                    )
                elif act_name == "move":
                    print(
                        f"You travelled to {color_text(get_obj_name(args[2], quest_data), COLOR_CYAN)}."
                    )
            ...
            print(
                color_text(
                    "Invalid input. Please enter a number or command letter.", COLOR_RED
                )
            )


def show_quest_image(json_path):
    """
    Opens quest image if exists.
    Windows: os.startfile
    """
    quest_file = os.path.basename(json_path)  # quest_1.json
    quest_id = quest_file.replace(".json", "")  # quest_1

    campaign_dir = os.path.dirname(json_path)
    image_path = os.path.join(campaign_dir, "images", f"{quest_id}.png")

    if os.path.exists(image_path):
        try:
            print(f"\n[IMAGE] Opening scene visualization: {image_path}\n")
            os.startfile(image_path)  # Windows only
        except Exception as e:
            print(f"[WARN] Could not open image: {e}")
    else:
        print("[IMAGE] No scene image found, continuing...\n")
...
def main():
    ...
    if not os.path.exists(quests_dir):
        print(
            f"Error: Directory '{quests_dir}' not found. Please run generate_story.py first."
        )
        sys.exit(1)
    ...
    if os.path.exists(quests_dir):
        subdirs = [
            ...
            if os.path.isdir(os.path.join(quests_dir, d)) and d != "raw"
        ]
    ...
    if len(campaigns) == 1:
        ...
        print(
            color_text(
                f"Only one campaign found, loading automatically: {selected_campaign['prompt']}",
                COLOR_YELLOW,
            )
        )
    else:
    ...
    print(
        color_text(
            "          VICTORY! YOU COMPLETED THE ENTIRE CAMPAIGN!          ",
